...
<title>${message("admin.shopAttribute.add")} - Powered By CETVISION</title>
...
$().ready(function() {
	...
	$type.change(function() {
		if ($type.val() == "select" || $type.val() == "checkbox") {
			$shopAttributeTable.find("tr.optionTr").remove();
			$addOptionTr.show();
			addOption();
		} else {
			$addOptionTr.hide();
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.shopAttribute.add")}
	</div>
	<form id="inputForm" action="save.jhtml" method="post">
		<table id="shopAttributeTable" class="input">
			...
			<tr>
				...
				<td>
					<select id="type" name="type">
						<option value="text">${message("ShopAttribute.Type.text")}</option>
						<option value="select">${message("ShopAttribute.Type.select")}</option>
						<option value="checkbox">${message("ShopAttribute.Type.checkbox")}</option>
					</select>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<label>
						<input type="checkbox" name="isEnabled" value="true" checked="checked" />${message("ShopAttribute.isEnabled")}
						<input type="hidden" name="_isEnabled" value="false" />
					</label>
					<label>
						<input type="checkbox" name="isRequired" value="true" />${message("ShopAttribute.isRequired")}
