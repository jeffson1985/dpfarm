...
<title>${message("admin.setting.edit")} - Powered By CETVISION</title>
...
$().ready(function() {
	...
	var $smsApiHost = $("#smsApiHost");
	var $smsApiKey = $("#smsApiKey");
	var $smsAccount = $("#smsAccount");
	var $smsPassword = $("#smsPassword");
	var $toSmsWrap = $("#toSmsWrap");
	var $toSmsPhone = $("#toSmsPhone");
	var $smsTest = $("#smsTest");
	var $smsTestStatus = $("#smsTestStatus");
	...
	$smsTest.click(function() {
		var $this = $(this);
		if ($this.val() == "${message("admin.setting.smsTest")}") {
			$this.val("${message("admin.setting.sendSms")}");
			$toSmsWrap.show();
		} else {
			function valid(element) {
				return $inputForm.validate().element(element);
			...
			$.ajax({
				url: "sms_test.jhtml",
				type: "POST",
				data: {smsApiHost: $smsApiHost.val(), smsApiKey: $smsApiKey.val(), smsAccount: $smsAccount.val(), smsPassword: $smsPassword.val(), toSmsPhone: $toSmsPhone.val()},
				dataType: "json",
				cache: false,
				beforeSend: function() {
					if (valid($smsApiHost) & valid($smsApiKey) & valid($smsAccount) & valid($toSmsPhone)) {
						$smsTestStatus.html('<span class="loadingIcon">&nbsp;<\/span>${message("admin.setting.sendSmsLoading")}');
						$this.prop("disabled", true);
					} else {
						return false;
					}
...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post" enctype="multipart/form-data">
		...
		<table class="input tabContent">
			<tr>
				<th>
					<span class="requiredField">*</span>${message("Setting.smsApiKey")}:
				</th>
				<td>
					<input type="text" id="smsApiKey" name="smsApiKey" class="text" value="${setting.smsApiKey}" maxlength="300" />
				</td>
			</tr>
			<tr>
				<th>
					<span class="requiredField">*</span>${message("Setting.smsApiHost")}:
				</th>
				<td>
					<input type="text" id="smsApiHost" name="smsApiHost" class="text" value="${setting.smsApiHost}" maxlength="300" />
				</td>
			</tr>
			<tr>
				<th>
					<span class="requiredField">*</span>${message("Setting.smsAccount")}:
				</th>
				<td>
					<input type="text" id="smsAccount" name="smsAccount" class="text" value="${setting.smsAccount}" maxlength="200" />
				</td>
			</tr>
			<tr>
				<th>
					${message("Setting.smsPassword")}:
				</th>
				<td>
					<input type="password" id="smsPassword" name="smsPassword" class="text" maxlength="200" title="${message("admin.setting.smsPasswordTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
					${message("admin.setting.smsTest")}:
				</th>
				<td>
					<span class="fieldSet">
						<span id="toSmsWrap" class="hidden">
							${message("admin.setting.toSmsPhone")}: <br />
							<input type="text" id="toSmsPhone" name="toSmsPhone" class="text ignore" maxlength="200" />
						</span>
						<input type="button" id="smsTest" class="button" value="${message("admin.setting.smsTest")}" />
						<span id="smsTestStatus">&nbsp;</span>
					</span>
				</td>
			</tr>
		</table>
