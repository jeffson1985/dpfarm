...
<%@page import="com.dp2345.CommonAttributes"%>
<%@page import="com.dp2345.util.JsonUtils"%>
...
<%
	...
	if (status.equals("success")) {
		...
		try {
			Resource resource = new ClassPathResource(CommonAttributes.DP2345_PROPERTIES_PATH);
			...
			status = "error";
			message = "DP2345.PROPERTIES文件写入失败!";
			exception = stackToString(e);
		}
	}
	...
	if (status.equals("success")) {
		try {
			...
			Element siteUrlElement = (Element) document.selectSingleNode("/dp2345/setting[@name='siteUrl']");
			Element logoElement = (Element) document.selectSingleNode("/dp2345/setting[@name='logo']");
			Element defaultLargeProductImageElement = (Element) document.selectSingleNode("/dp2345/setting[@name='defaultLargeProductImage']");
			Element defaultMediumProductImageElement = (Element) document.selectSingleNode("/dp2345/setting[@name='defaultMediumProductImage']");
			Element defaultThumbnailProductImageElement = (Element) document.selectSingleNode("/dp2345/setting[@name='defaultThumbnailProductImage']");
			String siteUrl;
	...
	if (status.equals("success")) {
		try {
			FileUtils.writeStringToFile(new File(rootPath + INSTALL_LOCK_CONFIG_PATH), "DP2345 INSTALL LOCK - DP2345.COM");
		} catch (IOException e) {
			status = "error";
	...
	if (status.equals("success")) {
		try {
			FileUtils.writeStringToFile(new File(rootPath + INSTALL_INIT_CONFIG_PATH), "DP2345 INSTALL INIT - DP2345.COM");
